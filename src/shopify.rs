...
impl <U : IntoUrl + Clone> ShopifyClient<U> {
    ...
    pub async fn get_all_products(&self) -> Result<ProductResponse, reqwest::Error> {
        let mut url = self.url.clone().into_url()?;
        url.set_path("/products.json");

        let response = self.req_client.get(url)
            .send()
            .await?
            .json()
            .await?;
    ...
    pub fn get_product_link(&self, product: &Product) -> Result<String, reqwest::Error> {
        let mut url = self.url.clone().into_url()?;
